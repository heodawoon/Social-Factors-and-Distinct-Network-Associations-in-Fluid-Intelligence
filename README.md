...
### Regression analyses — `R_script/`
Regression-based analyses for estimating associations and treatment effects:
- logistic regression with SHAP-selected covariates for individual feature associations with Gf (Fig. 2c, Fig. 4b, Supplementary Table 6)
- average treatment effect (ATE) estimation using generalized random forest models (Fig. 4a)
...
```
5. **Regression and statistical analyses**:
```bash
Rscript R_script/Logistic_regression.R
Rscript R_script/ATE.R
Rscript R_script/Descriptive_table.R
```
...
6. Run regression and statistical analyses (`R_script/Logistic_regression.R`, `R_script/ATE.R`, `R_script/Descriptive_table.R`).
